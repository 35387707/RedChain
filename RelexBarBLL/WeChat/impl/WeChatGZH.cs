...
{
    public class WeChatGZH : WeChatAPI
    {
        ...
        public WxPayData Unifiedorder(string gzhAppid, string MCHID,string MCHIDKEY, string requestIP, WxPayData inputObj)
        {
            ...
            Log.Debug("WeChatGZH", "UnfiedOrder request : " + xml);
            string response = HttpService.Post(xml, url, false, 20);
            Log.Debug("WeChatGZH", "UnfiedOrder response : " + response);
            ...
            ReportCostTime(gzhAppid,MCHID,requestIP,MCHIDKEY,url, timeCost, result);//测速上报
       ...
       */
        private static void ReportCostTime(string appid,string mchid,string ip,string MCHIDKEY, string interface_url, int timeCost, WxPayData inputObj)
        {
            //如果不需要进行上报
            ...
            try
            {
                Report(appid,mchid,ip,MCHIDKEY, data);
            }
            catch (Exception ex)
            {
                //不做任何处理
	    ...
	    */
        public static WxPayData Report(string appid,string mchid,string ip,string MCHIDKEY, WxPayData inputObj, int timeOut = 1)
        {
            ...
            Log.Info("WeChatGZH", "Report request : " + xml);

            string response = HttpService.Post(xml, url, false, timeOut);

            Log.Info("WeChatGZH", "Report response : " + response);
        ...
        public WxPayData OrderQuery(string gzhAppid, string MCHID,string MCHIDKEY,string ip, WxPayData inputObj, int timeOut = 6)
        {
            ...
            Log.Debug("WeChatGZH", "OrderQuery request : " + xml);
            string response = HttpService.Post(xml, url, false, timeOut);//调用HTTP通信接口提交数据
            Log.Debug("WeChatGZH", "OrderQuery response : " + response);
            ...
            ReportCostTime(gzhAppid,MCHID,ip,MCHIDKEY,url, timeCost, result);//测速上报

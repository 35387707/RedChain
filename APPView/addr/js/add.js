...
function judgReqFld(fromPage) {
    ...
    var areacode = trim($('#areacode').data('areacode'));
    if (areacode == '') {
        req_arr.push('所在地区');
    }
    //区域id, 也就是第三层的id
 ...
 * 接口.54.新增收货地址
 */
function addUserAddress(fromPage) {
    ...
    //登录的逻辑
    appcan.ajax({
        url : ajaxUrl + "Account/AddUserAddress",
        type : "POST",
        data : {
            recname : recname,
            areid : areid,
        ...
        success : function(data) {
            ...
            if (data.length != 0) {
                ...
                if (data.code == 1 || data.code == '1') {
                    ...
                    updtAddrSql(localStorage.getItem("uid"), data.msg, fromPage);
    ...
    if (temp == false) {
        ...
        uexDataBaseMgr.transactionEx(db, JSON.stringify(sql_tx), function(error) {
            ...
            if (error == 0) {
                ...
                setDefAddr(db, uid, addrid, [], fromPage);
...
function setDefAddr(db, uid, addrid, sql_tx, fromPage) {

    var sql = "update tbl_address set defaultAddr=1 where uid = '" + uid + "' and addrid = '" + addrid + "'";
    sql_tx.push(sql);
